...
// Função para lidar com usuário inativo
async function handleInactiveUser(client, from) {
  const horaAtual = new Date().getHours();
  let opcaoFalarCom = "Falar com a Dani 💬";
  if (horaAtual >= 16 && horaAtual <= 19) {
    opcaoFalarCom = "Falar com a Brielle 💬";
  ...
  await client.sendMessage(from, { text: `Aguarde um minuto que ${opcaoFalarCom} já fala com você.` });
}
